package com.smart.pay.channel.basechannel;

import com.smart.pay.channel.async.AsyncMethod;
	...
	public String whetherRepeatOrder(String orderCode) {
		...
		if(findByOrderCode.getWhetherToRequest() == 1) {
			...
			if(CommonConstants.SUCCESS.equals(respCode)) {
				...
				requestEntity.add("amount", prp.getAmount().toString());
				...
				PaymentOrder bean = (PaymentOrder) JSONObject.toBean(jsonObj, PaymentOrder.class);
				bean.setExtra(extra);
