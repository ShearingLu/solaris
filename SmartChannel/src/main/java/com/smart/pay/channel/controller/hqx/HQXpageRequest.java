package com.smart.pay.channel.controller.hqx;
...
//		}
	 ...
	 * @return
	 */
	@RequestMapping(method = RequestMethod.POST, value = "/smartchannel/topup/hqx/hqxRegister")
	public @ResponseBody
    Object hqToRegister(@RequestParam(value = "bankCard") String bankCard,
                        ...
                        @RequestParam(value = "extraFee") String extraFee,@RequestParam(value="merchant_id") String merchantId) {
		...
		LOG.info("=============环球小额落地进件返回信息========：" + resultMap.toString());
...
//			topupPayChannelBusiness.createHQXRegister(hqRegister);
			...
			String channelTag="HQX_DH";
			//进件成功将费率及子商户号存入数据库
			String saveSubMerchantUrl="http://smartmerchant/smartmerchant/merchant/user/info/save";
			MultiValueMap params=new LinkedMultiValueMap<>();
			params.add("merchant_id",merchantId);//下游商户号
			params.add("channel_tag",channelTag);
			params.add("phone",phone);
			params.add("id_card",idCard);
			params.add("sub_merchant_id",subMerchantNo);
			params.add("sub_rate",rate);
			params.add("sub_extra_fee",extraFee);
			try {
				restTemplate.postForObject(saveSubMerchantUrl,params,String.class);
				LOG.info("进件信息保存成功========phone===="+phone+"===下游商户号==="+merchantId+"====进件子商户号==="+subMerchantNo);
			} catch (RestClientException e) {
				e.printStackTrace();
				LOG.info("进件信息保存失败========phone===="+phone+"===下游商户号==="+merchantId+"====进件子商户号==="+subMerchantNo);
			}
	...
	// 绑卡接口
	@RequestMapping(method = RequestMethod.POST, value = "/smartchannel/topup/hqx/bindCard")
	@ResponseBody
	public Object hqToBindCard(@RequestParam(value = "bankCard") String bankCard,
							   ...
							   @RequestParam(value="subMerchantNo") String subMerchantNo,
							   @RequestParam(value="merchant_id") String merchantId,
							   @RequestParam(value="dsorderid")String dsorderid) {
		LOG.info("开始绑卡==================");
...
//		String subMerchantNo = hqRegister.getMerchantCode();
		...
		param.put("expireDate", this.expiredTimeToMMYY(expiredTime)); // 信用卡有效期 格式：MMYY卡类型为贷记卡时必填
		param.put("returnUrl", "http://101.132.160.107/v1.0/paymentchannel/topup/wmyk/bindcardsuccess");// 前台跳转地址 TODO
		param.put("notifyUrl", ip + "/v1.0/paymentgateway/topup/hqx/bindcard/call_back");// 异步通知地址
		LOG.info("hqx绑卡请求参数=====================" + param.toString());
		Map<String, String> resultMap = smart.allRequestMethod(param);
...
//				topupPayChannelBusiness.createHQXBindCard(hqBindCard);
				...
				String channelTag="HQX_DH";
				//进件成功将费率及子商户号存入数据库
				String saveSubMerchantUrl="http://smartmerchant/smartmerchant/merchant/user/bindcard/save";
				MultiValueMap params=new LinkedMultiValueMap<>();
				params.add("merchant_id",merchantId);//下游商户号
				params.add("channel_tag",channelTag);
				params.add("sub_merchant_id",subMerchantNo);
				...
				try {
					restTemplate.postForObject(saveSubMerchantUrl,params,String.class);
					LOG.info("绑卡信息保存成功========phone===="+phone+"===下游商户号==="+merchantId+"====进件子商户号==="+subMerchantNo);
				} catch (RestClientException e) {
					e.printStackTrace();
					LOG.info("绑卡信息保存失败========phone===="+phone+"===下游商户号==="+merchantId+"====进件子商户号==="+subMerchantNo);
				}
				map.put(CommonConstants.RESP_CODE, CommonConstants.SUCCESS);
				map.put(CommonConstants.RESP_MESSAGE, "绑卡成功");
				map.put(CommonConstants.RESULT,errtext);
				return map;
			}
			// 跳转绑卡银联页面
...
//				}
	 ...
	 * @return
	 */
	@RequestMapping(method = RequestMethod.POST, value = "/smartchannel/topup/hqx/topay")
	public Object topay(@RequestParam(value = "orderCode") String orderCode) throws Exception {
		...
		BigDecimal amount = prp.getAmount();
		String realAmount=amount.multiply(new BigDecimal("100")).setScale(0,BigDecimal.ROUND_DOWN).toString();
		String subMerchantNo=prp.getSubMerchantNo();
		String bindId=prp.getBindId();
		String notifyUrl=prp.getNotifyURL();
...
//		String subMerchantNo = hqRegister.getMerchantCode();
		...
		Date date = new Date();
		SimpleDateFormat format = new SimpleDateFormat("yyyyMMddHHmmss");
		String ordersn = "byt" + format.format(date);
		param.put("ordersn", ordersn); // 流水号
		param.put("dsorderid", orderCode); // 商户订单号
		param.put("subMerchantNo", subMerchantNo);// 子商户号
		param.put("bindId", bindId); // TODO bindCard 返回 绑卡标识
		param.put("bankcard", bankCard); // 银行卡号
		param.put("amount", realAmount); // 单位（分） 金额
		...
		param.put("notifyUrl", notifyUrl); // 异步通知地址
		Map<String, String> resultMap = smart.allRequestMethod(param);
		LOG.info("=============环球小额落地：" + resultMap.toString());
		String errtext = resultMap.get("errtext");
		String code = resultMap.get("returncode"); // 返回码
	...
	public void fayNotify(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		PaymentOrder prp = redisUtil.getPaymentRequestParameter(dsorderid);
	 ...
	 * @return
	 */
	@RequestMapping(method = RequestMethod.POST, value = "/smartchannel/topup/hqx/transfer")
	 ...
	 */
	@ResponseBody
	@RequestMapping(method = RequestMethod.POST, value = "/smartchannel/topup/hqx/balanceQuery")
	public Object balanceQuery(@RequestParam(value = "subMerchantNo") String subMerchantNo,
							   @RequestParam(value = "bindId") String bindId) throws Exception {
		LOG.info("开始进入商户余额查询========================");
		Map<String, Object> map = new HashMap<>();
		...
		if ("0000".equals(code)) {
			map.put(CommonConstants.RESP_CODE, CommonConstants.SUCCESS);
			map.put(CommonConstants.RESP_MESSAGE, "当前可用余额:" + currAccountBalance);
			return map;
