...
import com.smart.pay.channel.controller.yzf.YZFpayRequest;
...
@Service
public class YZFPaymentRequestBusinessImpl extends BasePaymentRequest implements PaymentRequestBusiness {

    private static final Logger LOG = LoggerFactory.getLogger(YZFPaymentRequestBusinessImpl.class);

    @Autowired
    private HttpServletRequest request;

    @Autowired
    private YZFpayRequest yzFpayRequest;
    ...
    public Map<String, Object> paymentRequest(Map<String, Object> params) throws Exception {
        ...
        LOG.info("飏支付订单请求参数============"+paymentOrder);
...
//            }
            // 用户进入消费任务
            map = (Map<String, Object>) yzFpayRequest.toPay(orderCode);
        }

        if ("11".equals(orderType)) {
            // 用户进入还款任务
            map = (Map<String, Object>) yzFpayRequest.transfer(orderCode);
        }
